...
const App = () => {
  ...
  const addPerson = event => {
    ...
    if (nameIsDuplicate && !!newNumber) {
      let replaceNumberBool = window.confirm(
        `${newName} is already added to phonebook, replace the old number with a new one?`
      );
      if (replaceNumberBool) {
        personService.getAll().then(allPersons => {
          let duplicatePerson = allPersons.filter(
          ...
          personService
            ...
            .then(returnedPerson => {
              ...
              setNotifyMessage(
                `Updated phone number of ${returnedPerson.name}.`
              );
              setTimeout(() => {
            ...
            .catch(error => {
              ...
              setPersons(
                persons.filter(person => person.id !== duplicatePerson[0].id)
              );
              setTimeout(() => {
                setNotifyMessage(null);
              }, 5000);
  ...
  const personsToBeShown = persons.filter(person =>
